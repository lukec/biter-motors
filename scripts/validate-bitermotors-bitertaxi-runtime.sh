...
setup = r'''/sc game.tick_paused=false local s=game.surfaces.nauvis local f=game.forces.player local offices=remote.call("bitermotors","sales_office_status","player") local served=offices[1] and offices[1].settlements[1] local office=s.find_entities_filtered{name="bitermotors-sales-office",force=f}[1] local base=served and served.position or office and office.position or {x=0,y=0} local function make(dx) local p=s.find_non_colliding_position("bitermotors-bitertaxi-depot",{base.x+dx,base.y+20},128,1) local c=s.create_entity{name="bitermotors-bitertaxi-depot",position=p,force=f} script.raise_event(defines.events.script_raised_built,{entity=c}) local inv=c.get_inventory(defines.inventory.chest) inv.insert{name="bitermotors-bitertaxi-fleet",count=200} local pole=s.create_entity{name="substation",position={p.x,p.y+6},force=f} local power=s.create_entity{name="electric-energy-interface",position={p.x+3,p.y+6},force=f} power.electric_interface_mode=defines.electric_interface_mode.primary_output power.power_production=30000000 power.output_flow_limit=30000000 return c end local c1=make(0) local c2=make(12) local inv2=c2.get_inventory(defines.inventory.chest) inv2[41].set_stack{name="bitermotors-dollar",count=100000} s.create_entity{name="bitermotors-bitertaxi-depot-power",position=c1.position,force=f} s.create_entity{name="bitermotors-bitertaxi-depot-power",position={base.x+300,base.y+300},force=f} storage.bitermotors_rsc_test={c1=c1.unit_number,c2=c2.unit_number,surface=s.index} rcon.print("RSC_SETUP ok")'''
query = r'''/sc local rows=remote.call("bitermotors","bitertaxi_depot_status","player") local s=game.get_surface(storage.bitermotors_rsc_test.surface) local customers=#s.find_entities_filtered{type="unit",force=game.forces["bitermotors-customers"]} local helper_count=#s.find_entities_filtered{name="bitermotors-bitertaxi-depot-power",force=game.forces.player} rcon.print("RSC_RESULT "..helpers.table_to_json{rows=rows,customers=customers,helpers=helper_count})'''
destroy = r'''/sc local s=game.get_surface(storage.bitermotors_rsc_test.surface) for _,c in pairs(s.find_entities_filtered{name="bitermotors-bitertaxi-depot",force=game.forces.player}) do if c.unit_number==storage.bitermotors_rsc_test.c2 then c.destroy{raise_destroy=true} end end rcon.print("RSC_DESTROY ok")'''
