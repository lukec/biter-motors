...
cat > "$mods/mod-list.json" <<'EOF_MOD_LIST'
{
  "mods": [
    ...
    {"name": "bitermotors_smoke", "enabled": true}
  ]
}
EOF_MOD_LIST
...
data:extend({
  {
    type = "recipe",
    name = "bitermotors-smoke-premium-ev-economics",
    localised_name = {"recipe-name.bitermotors-premium-ev"},
    categories = {"bitermotors-vehicle-assembly"},
    enabled = true,
    hidden = true,
    energy_required = 20,
...
local REPORT = "bitermotors-smoke.jsonl"
local CONTROLLER = "bitermotors-planetary-grid-controller"
local AGI_MODEL = "bitermotors-agi-model"
local AGI_RECIPE = "bitermotors-agi-training-run"
local DOLLAR = "bitermotors-dollar"
local GRID_CONNECTION = "bitermotors-ev-charging-grid-connection"
local POWER_SINK = "bitermotors-ev-charging-power-sink"
local V2_POWER_SINK = "bitermotors-ev-charging-v2-power-sink"
local SALES_OFFICE = "bitermotors-sales-office"
local STATION = "bitermotors-ev-charging-station"
local STATION_V2 = "bitermotors-ev-charging-station-v2"
local STATION_V3 = "bitermotors-ev-charging-station-v3"
local STATION_V4 = "bitermotors-ev-charging-station-v4"
local POWER_POLE = "medium-electric-pole"
local POWER_SOURCE = "electric-energy-interface"
local RESERVATION = "bitermotors-ev-reservation"
local FIRST_SALE_RECIPE = "bitermotors-sell-prototype-roadster"
...
local CUSTOMER_FORCE = "bitermotors-customers"
local TERRESTRIAL_DATACENTER = "bitermotors-terrestrial-datacenter"
...
script.on_init(function()
  ...
  local cybertrain_rail
  for y = -20, 20, 2 do
    cybertrain_rail = surface.create_entity{
      name = "straight-rail", position = {220, y}, direction = defines.direction.north, force = force
    } or cybertrain_rail
  end
  local cybertrain_position = surface.find_non_colliding_position(CYBERTRAIN, {220, 0}, 20, 0.5)
  local cybertrain = cybertrain_position and surface.create_entity{
    name = CYBERTRAIN, position = cybertrain_position, direction = defines.direction.north, force = force
  }
  local cybertrain_stop = surface.create_entity{
    name = CYBERTRAIN_CHARGING_STOP, position = {222, 0}, direction = defines.direction.west, force = force
  }
  storage.cybertrain_created = cybertrain ~= nil
  storage.cybertrain_stop_created = cybertrain_stop ~= nil
  ...
  remote.call("bitermotors", "refresh_biter_customer_market", "player")
  ...
  storage.road_rage_test = remote.call(
    "bitermotors", "test_customer_road_rage", commanded_biter.unit_number, customer_turret, 120
  )
...
script.on_nth_tick(30, function()
  ...
  if roadster.burner and roadster.burner.inventory then
    drive_charge = roadster.burner.inventory.get_item_count("bitermotors-electric-drive-charge") * 1000000
...
script.on_nth_tick(60, function()
  if not storage.preproduction_market then
    storage.preproduction_market = remote.call("bitermotors", "biter_customer_market", "player")
  end
end)
...
script.on_nth_tick(300, function()
  ...
  if storage.customer_ev_seeded and not storage.postproduction_market then
    ...
    storage.postproduction_market = remote.call("bitermotors", "biter_customer_market", "player")
  end
end)
...
script.on_nth_tick(3440, function()
  ...
  local before = remote.call("bitermotors", "sales_office_status", "player")
  if settlement then settlement.destroy() end
  local after = remote.call("bitermotors", "sales_office_status", "player")
  local performance = remote.call("bitermotors", "performance_status", "player")
  write_report{
    tick = game.tick,
...
script.on_nth_tick(3780, function()
  ...
  local station_recipe = game.forces.player.recipes["bitermotors-ev-charging-station"]
  local station_v2_recipe = game.forces.player.recipes["bitermotors-ev-charging-station-v2"]
  local station_v3_recipe = game.forces.player.recipes["bitermotors-ev-charging-station-v3"]
  ...
  local agi_training = remote.call("bitermotors", "agi_training_status", "player")
  local cybertrains = remote.call("bitermotors", "cybertrain_status", "player")
  ...
  local road_rage_units = game.forces["bitermotors-road-rage"] and #surface.find_entities_filtered{
    type = "unit", force = game.forces["bitermotors-road-rage"]
  } or 0
  local prospect_units = #surface.find_entities_filtered{
    name = {
      "bitermotors-small-biter-prospect", "bitermotors-medium-biter-prospect",
      "bitermotors-big-biter-prospect", "bitermotors-behemoth-biter-prospect",
      "bitermotors-small-spitter-prospect", "bitermotors-medium-spitter-prospect",
      "bitermotors-big-spitter-prospect", "bitermotors-behemoth-spitter-prospect"
    },
  ...
  write_report{
    ...
    cybertrain_created = storage.cybertrain_created,
...
script.on_nth_tick(8000, function()
  ...
  write_report{
    ...
    commutes = remote.call("bitermotors", "customer_charging_commutes", "player"),
    performance = remote.call("bitermotors", "performance_status", "player")
  }
end)
...
for reference in sorted(set(re.findall(r'"(__[^"]+/graphics/[^"]+)"', data_file.read_text()))):
    ...
    actual = actual.replace("__bitermotors__", str(mod_dir))
...
bitermotors_badge = "__bitermotors__/graphics/technology/bitermotors-tech-badge.png"
bitermotors_technologies = {
...
for technology_name in bitermotors_technologies:
    icon_paths = {layer.get("icon") for layer in data["technology"][technology_name].get("icons", [])}
    if bitermotors_badge not in icon_paths:
...
for technology_name, expected_count in module_two_counts.items():
    ...
    if ingredients != rgb | {"bitermotors-dollar"} or technology["unit"]["count"] != expected_count:
    ...
    if "bitermotors-sales-office" not in prerequisites or "space-science-pack" in prerequisites:
...
for module_family in ("speed", "productivity", "efficiency", "quality"):
    ...
    expected_ingredients = {
        module_2: 4, "advanced-circuit": 5, "processing-unit": 5, "bitermotors-dollar": 10,
    }
    ...
    if technology.get("hidden") or set(technology.get("prerequisites", [])) != {module_2, "bitermotors-capital-scaling"}:
        raise SystemExit(f"{module_3} Nauvis technology mismatch: {technology}")

if set(data["technology"]["epic-quality"].get("prerequisites", [])) != {"quality-module-3", "bitermotors-terrestrial-ai"}:
...
expected_battery_pack_recipes = {
    "bitermotors-high-energy-battery-pack": {
        "bitermotors-high-nickel-cell": 4, "steel-plate": 4, "advanced-circuit": 2,
    },
    "bitermotors-lfp-battery-pack": {
        "bitermotors-lfp-cell": 4, "steel-plate": 4, "electronic-circuit": 2,
    },
}
...
cell_scale_high_nickel = data["recipe"]["bitermotors-cell-scale-high-nickel"]
cell_scale_inputs = {
    row["name"]: row["amount"] for row in cell_scale_high_nickel["ingredients"]
}
if cell_scale_inputs != {
    "bitermotors-nickel-sulfate": 4,
    "bitermotors-lithium-carbonate": 1,
    "bitermotors-battery-graphite": 2,
...
expected_battery_recovery = {
    "bitermotors-high-energy-battery-recovery": (
        "bitermotors-damaged-high-energy-battery-pack", "bitermotors-high-nickel-cell", 36,
    ),
    "bitermotors-lfp-battery-recovery": (
...
for technology_name in ("bitermotors-industrial-supply-chain", "big-mining-drill", "recycling"):
...
if foundry_packs != {
    "automation-science-pack", "logistic-science-pack", "chemical-science-pack", "bitermotors-dollar"
...
expected_branch_prerequisites = {
    "bitermotors-industrial-supply-chain": {"automation-2", "electric-mining-drill", "steel-processing"},
    "big-mining-drill": {"bitermotors-industrial-supply-chain", "engine"},
    "foundry": {"bitermotors-industrial-supply-chain", "bitermotors-energy-products", "concrete"},
    "recycling": {"bitermotors-industrial-supply-chain", "concrete"},
}
...
if electric_furnace_owners != {"bitermotors-industrial-supply-chain"}:
...
ev_production_line = data["technology"]["bitermotors-premium-ev-program"]
ev_line_unlocks = {
...
premium_sale = data["recipe"]["bitermotors-sell-premium-ev"]
mass_market_sale = data["recipe"]["bitermotors-sell-mass-market-ev"]
prototype_sale = data["recipe"]["bitermotors-sell-prototype-roadster"]
if prototype_sale["energy_required"] != 60 or prototype_sale["results"] != [{"type": "item", "name": "bitermotors-dollar", "amount": 2}]:
    raise SystemExit(f"Prototype Roadster sale balance mismatch: {prototype_sale}")
if premium_sale["energy_required"] != 30 or premium_sale["results"] != [{"type": "item", "name": "bitermotors-dollar", "amount": 1}]:
...
if set(terrestrial_tech["prerequisites"]) != {"bitermotors-capital-scaling", "bitermotors-energy-products", "processing-unit"}:
    raise SystemExit(f"Terrestrial AI prerequisites mismatch: {terrestrial_tech['prerequisites']}")
autonomy_tech = data["technology"]["bitermotors-autonomous-logistics"]
...
orbital_unlocks = {
    effect.get("recipe") for effect in orbital_tech.get("effects", [])
    if effect.get("type") == "unlock-recipe"
}
...
radiator = data["solar-panel"]["bitermotors-orbital-radiator-panel"]
...
if not any(result.get("name") == "bitermotors-ai-token" and result.get("amount") == 20 for result in token_results):
...
if data["item"]["bitermotors-ai-token"].get("weight") != 1:
...
missing_unlocks = bitermotors_recipes - technology_unlocks - runtime_milestone_recipes
if missing_unlocks:
...
uncraftable = {}
for recipe_name in sorted(bitermotors_recipes):
...
chargers = {
    "V2": (
        data["logistic-container"]["bitermotors-ev-charging-station-v2"],
        data["electric-energy-interface"]["bitermotors-ev-charging-v2-power-sink"],
        [[-2, -2], [2, 2]],
        "150kW",
    ),
    "V3": (
        data["logistic-container"]["bitermotors-ev-charging-station-v3"],
        data["electric-energy-interface"]["bitermotors-ev-charging-v3-power-sink"],
        [[-2.5, -2.5], [2.5, 2.5]],
        "250kW",
    ),
    "V4": (
        data["logistic-container"]["bitermotors-ev-charging-station-v4"],
        data["electric-energy-interface"]["bitermotors-ev-charging-v4-power-sink"],
        [[-3, -3], [3, 3]],
        "500kW",
    ),
}
...
if shortcut["action"] != "lua" or not shortcut["toggleable"] or shortcut["technology_to_unlock"] != "bitermotors-sales-office":
...
for vehicle_name in (
    ...
    for frame_index in range(1, 9):
        sprite_name = f"bitermotors-sales-office-showroom-{vehicle_name}-frame-{frame_index}"
...
for vehicle_name in (
    ...
    layer = layers[0]
    asset_name = vehicle_name.removeprefix("bitermotors-")
    expected_file = f"__bitermotors__/graphics/entity/vehicles/{asset_name}.png"
    ...
    expected_shadow = f"__bitermotors__/graphics/entity/vehicles/{asset_name}-shadow.png"
...
cybertrain_status = checked.get("cybertrain_status") or []
if len(cybertrain_status) != 1 or cybertrain_status[0].get("battery_percent") != 100:
    raise SystemExit(f"electric Semi was not registered with a full factory battery: {checked}")
reserve_test = checked.get("cybertrain_reserve_test") or {}
