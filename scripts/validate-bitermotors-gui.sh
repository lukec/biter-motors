...
if [[ -z "$source_save" || ! -f "$source_save" ]]; then
  echo "Usage: $0 /path/to/bitermotors-save.zip" >&2
  exit 2
fi
...
cleanup() {
  local status=$?
  if (( status != 0 )) && [[ -f "$tmp/bitermotors-gui-benchmark.log" ]]; then
    tail -80 "$tmp/bitermotors-gui-benchmark.log" >&2
  fi
  rm -rf "$tmp"
  return "$status"
}
...
cat > "$mods/mod-list.json" <<'EOF_MOD_LIST'
{
  "mods": [
    ...
    {"name": "bitermotors_gui_smoke", "enabled": true}
  ]
}
EOF_MOD_LIST
...
local function dollar_diagnostics(player)
  ...
  local offices = {}
  for _, surface in pairs(game.surfaces) do
    for _, office in pairs(surface.find_entities_filtered{name = "bitermotors-sales-office", force = player.force}) do
...
script.on_event(defines.events.on_tick, function()
  ...
  local progress_ok, progress_result = pcall(function()
    return remote.call("bitermotors", "open_progress", player.index)
  end)
  local progress_status = remote.call("bitermotors", "progress_status", player.force.name)
  local market_status = remote.call("bitermotors", "refresh_biter_customer_market", player.force.name)
  local integrity_ok, integrity_result = pcall(function()
    return remote.call("bitermotors", "progression_integrity", player.force.name)
  end)
  local office = first_entity(player, "bitermotors-sales-office")
  local office_open_ok, office_open_error = pcall(function()
    if office then
      player.opened = office
      remote.call("bitermotors", "open_entity_info", player.index, office)
    end
  end)
  local sales_office_panel_created = player.gui.relative.bitermotors_entity_info_panel ~= nil
  ...
  if office then
    ...
    custom_power_alert_ok = pcall(function()
      player.add_custom_alert(
        office,
        {type = "item", name = "bitermotors-sales-office"},
    ...
    if station_position then
      charger = surface.create_entity{
        name = "bitermotors-ev-charging-station",
      ...
      if charger then
        ...
        remote.call("bitermotors", "open_entity_info", player.index, charger)
        charger_panel_created = player.gui.screen.bitermotors_station_info_panel ~= nil
      end
    end
    ...
    remote.call("bitermotors", "progression_integrity", player.force.name)
    ...
    if datacenter_position then
      datacenter = surface.create_entity{
        name = "bitermotors-terrestrial-datacenter",
        position = datacenter_position,
        force = player.force
      }
      if datacenter then
        datacenter.set_recipe("bitermotors-terrestrial-ai-token")
        datacenter_recipe_selected = datacenter.get_recipe()
          and datacenter.get_recipe().name == "bitermotors-terrestrial-ai-token"
        player.opened = datacenter
        remote.call("bitermotors", "open_entity_info", player.index, datacenter)
  ...
  write_report{
    ...
    prototype_roadster_enabled = player.force.recipes["bitermotors-prototype-roadster"].enabled
  }
  storage.complete = true
end)
EOF_LUA
