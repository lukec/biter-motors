...
script.on_init(function()
  ...
  helpers.write_file("bitermotors-fresh-start.jsonl", helpers.table_to_json{
    ...
    sales_office_researched = force.technologies["bitermotors-sales-office"].researched,
    prototype_roadster_enabled = force.recipes["bitermotors-prototype-roadster"].enabled
  } .. "\n", true)
end)
EOF
...
if "BITERMOTORS" not in str(row["intro"]) or "Recover the scattered cargo" not in str(row["intro"]):
