...
```bash
scripts/build-bitermotors-art.py
scripts/build-bitermotors-art-qa.py
```

The first command derives normalized icons from immutable files in
`art/bitermotors-icon-sources/`, technology illustrations, and small animation
overlays without image generation. The second rebuilds the static QA index at
`art/bitermotors-qa/index.html`. Both builders are deterministic and idempotent.
