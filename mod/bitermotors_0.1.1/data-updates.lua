...
local function mark_bitermotors_technology(technology, icon)
  technology.icon = nil
...
data:extend({
  {
    type = "technology",
    name = "bitermotors-industrial-supply-chain",
    icons = {
      {icon = "__base__/graphics/technology/automation-2.png", icon_size = 256},
      {icon = "__bitermotors__/graphics/technology/bitermotors-tech-badge.png", icon_size = 64, scale = 0.78, shift = {88, 88}}
    },
...
mark_bitermotors_technology(big_drill_tech, "__space-age__/graphics/technology/big-mining-drill.png")
big_drill_tech.prerequisites = {"bitermotors-industrial-supply-chain", "engine"}
...
mark_bitermotors_technology(foundry_tech, "__space-age__/graphics/technology/foundry.png")
foundry_tech.prerequisites = {"bitermotors-industrial-supply-chain", "bitermotors-energy-products", "concrete"}
...
mark_bitermotors_technology(recycling_tech, "__recycler__/graphics/technology/recycling.png")
recycling_tech.prerequisites = {"bitermotors-industrial-supply-chain", "concrete"}
...
recycling_tech.effects[#recycling_tech.effects + 1] = unlock("bitermotors-wrecked-ev-recycling")
...
rewrite_recipe("tesla-turret", {
  ...
  ingredients = ingredients(
    {"teslagun", 1},
    {"bitermotors-high-energy-battery-pack", 10},
    {"processing-unit", 20},
    {"accumulator", 4}
  )
})
rewrite_recipe("tesla-ammo", {
  categories = {"advanced-crafting"},
  ingredients = ingredients(
    {"bitermotors-high-nickel-cell", 1},
    {"advanced-circuit", 2},
    {"copper-cable", 10}
  ),
  energy_required = 5
...
mark_bitermotors_technology(tesla_tech, "__space-age__/graphics/technology/tesla-weapons.png")
tesla_tech.prerequisites = {"bitermotors-energy-products", "military-3", "processing-unit"}
...
mark_bitermotors_technology(logistic_system_tech, "__base__/graphics/technology/logistic-system.png")
logistic_system_tech.prerequisites = {"logistic-robotics", "bitermotors-industrial-supply-chain"}
...
for _, technology_name in ipairs({
  ...
  if technology then
    ...
    prerequisites[#prerequisites + 1] = "bitermotors-sales-office"
    ...
    for _, ingredient in ipairs(technology.unit.ingredients or {}) do
      ...
      if ingredient_name == "space-science-pack" then
        research_ingredients[#research_ingredients + 1] = {"bitermotors-dollar", ingredient.amount or ingredient[2] or 1}
      else
        research_ingredients[#research_ingredients + 1] = ingredient
      end
    end
    technology.unit.ingredients = research_ingredients
    mark_bitermotors_technology(technology, technology.icon)
  end
end
...
for _, module_family in ipairs({"speed", "productivity", "efficiency", "quality"}) do
  ...
  technology.prerequisites = {module_2, "bitermotors-capital-scaling"}
...
epic_quality.prerequisites = {"quality-module-3", "bitermotors-terrestrial-ai"}
...
legendary_quality.prerequisites = {"epic-quality", "bitermotors-orbital-compute"}
...
resource_autoplace.initialize_patch_set("bitermotors-nickel-ore", false)
...
data.raw.resource["bitermotors-nickel-ore"].autoplace = battery_mineral_autoplace({
  name = "bitermotors-nickel-ore",
  order = "d",
...
resource_autoplace.initialize_patch_set("bitermotors-lithium-brine", false)
data.raw.resource["bitermotors-lithium-brine"].autoplace = battery_mineral_autoplace({
  name = "bitermotors-lithium-brine",
  order = "e",
  base_density = 5.0,
...
nauvis.map_gen_settings.autoplace_controls["bitermotors-nickel-ore"] = {frequency = 1.0, size = 1.0, richness = 1.0}
nauvis.map_gen_settings.autoplace_controls["bitermotors-lithium-brine"] = {frequency = 1.0, size = 1.0, richness = 1.0}
