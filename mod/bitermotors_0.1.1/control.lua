...
local STATION_CONFIGS = {
  ["bitermotors-ev-charging-station"] = {
    ...
    power_sink_name = "bitermotors-ev-charging-power-sink"
  },
  ["bitermotors-ev-charging-station-v2"] = {
...
DAMAGED_LFP_PACK_NAME = "bitermotors-damaged-lfp-battery-pack"
BATTERY_RECOVERY_TECH_NAME = "bitermotors-battery-material-recovery"
CYBERTRAIN_NAME = "bitermotors-cybertrain"
CYBERTRAIN_FUEL_NAME = "bitermotors-cybertrain-drive-charge"
CYBERTRAIN_CHARGING_STOP_NAME = "bitermotors-cybertrain-charging-stop"
CYBERTRAIN_CHARGING_POWER_NAME = "bitermotors-cybertrain-charging-power"
CYBERTRAIN_BATTERY_CAPACITY = 1000000000
CYBERTRAIN_CHARGING_POWER = 50000000
CYBERTRAIN_KINETIC_ENERGY_SCALE = 10000
CYBERTRAIN_REGEN_EFFICIENCY = 0.65
CYBERTRAIN_PROCESS_BUDGET = 32
CYBERTRAIN_RESERVE_THRESHOLD = 10000000
CYBERTRAIN_RESERVE_SPEED = 0.08
local CUSTOMER_FORCE_NAME = "bitermotors-customers"
ROAD_RAGE_FORCE_NAME = "bitermotors-road-rage"
...
BITERMOTORS_COMPUTE_RECIPES = {
  ...
  ["bitermotors-planetary-grid-controller"] = AGI_TRAINING_RECIPE_NAME
}
local DOLLAR_NAME = "bitermotors-dollar"
local PROTOTYPE_ROADSTER_NAME = "bitermotors-prototype-roadster"
local PREMIUM_EV_NAME = "bitermotors-premium-ev"
local ADVANCED_BATTERY_CHEMISTRY_TECH_NAME = "bitermotors-advanced-battery-chemistry"
local ADVANCED_BATTERY_CHEMISTRY_RECIPES = {
  ...
  "bitermotors-high-energy-battery-pack",
...
SALES_OFFICE_SHOWROOM_SPRITES = {
  ...
  [MASS_MARKET_EV_SALE_RECIPE] = "bitermotors-sales-office-showroom-mass-market-ev-frame-",
  [MEGATRUCK_SALE_RECIPE] = "bitermotors-sales-office-showroom-megatruck-frame-"
}
EV_SALES_GATES = {
  premium = {
    item = "bitermotors-prototype-roadster",
    threshold = 50,
    technology = "bitermotors-premium-ev-program",
    recipes = {"bitermotors-premium-ev", "bitermotors-sell-premium-ev"},
    label = "Premium EV"
  },
  mass_market = {
    item = "bitermotors-premium-ev",
    threshold = 250,
    technology = "bitermotors-capital-scaling",
    recipes = {"bitermotors-mass-market-ev", "bitermotors-sell-mass-market-ev"},
    label = "Mass-market EV"
  },
  megatruck = {
    item = "bitermotors-mass-market-ev",
    threshold = 2000,
    technology = "bitermotors-megatruck-engineering",
...
BITERMOTORS_MAX_ATTACK_GROUP_SIZE = 80
BITERMOTORS_MIN_EXPANSION_COOLDOWN_TICKS = 10 * 60 * 60
BITERMOTORS_MAX_EXPANSION_COOLDOWN_TICKS = 60 * 60 * 60
...
AI_EFFICIENCY_TRACKS = {
  terrestrial = {
    ...
    technology = "bitermotors-terrestrial-ai",
    recipe = "bitermotors-terrestrial-ai-token",
    technology_prefix = "bitermotors-terrestrial-ai-efficiency-",
    tokens_per_cycle = 20
  },
  orbital = {
...
BITERMOTORS_START_DEBRIS_ITEMS = {
  ["iron-plate"] = 400,
  ["copper-plate"] = 200,
  ["stone"] = 200,
  ["coal"] = 200,
...
BITERMOTORS_ENERGY_JUMPSTART_QUALITY = "legendary"
BITERMOTORS_ENERGY_JUMPSTART_NORMAL_QUALITY_ITEMS = {
  ["passive-provider-chest"] = true,
  ["storage-chest"] = true
}
local BITERMOTORS_RUNTIME_VISUAL_CONFIGS = {
  ["bitermotors-sales-office"] = {
    status = true,
    sprite_prefix = "bitermotors-sales-office-status-red-frame-",
    working_sprite_prefix = "bitermotors-sales-office-status-green-frame-",
    warning_sprite_prefix = "bitermotors-sales-office-status-amber-frame-",
    stopped_sprite_prefix = "bitermotors-sales-office-status-red-frame-",
    offset = {1.03, -1.18},
    scale = 0.5
  },
...
CHARGER_STALL_VISUAL_LAYOUTS = {
  ...
  ["bitermotors-ev-charging-station-v2"] = {columns = 4, spacing_x = 0.78, spacing_y = 0.72, offset_y = -0.15, scale = 0.78},
  ["bitermotors-ev-charging-station-v3"] = {columns = 6, spacing_x = 0.76, spacing_y = 0.72, offset_y = -0.05, scale = 0.72},
...
CUSTOMER_VEHICLE_CLASS_BY_ITEM = {
  ["bitermotors-prototype-roadster"] = "roadster",
...
for _, base_name in pairs(CUSTOMER_UNIT_BASE_NAMES) do
  ...
  local prospect_name = "bitermotors-" .. base_name .. "-prospect"
  ...
  for _, class_name in pairs(CUSTOMER_VEHICLE_CLASS_BY_ITEM) do
    local variant_name = "bitermotors-" .. base_name .. "-" .. class_name
...
local function bitermotors_runtime_visuals()
  storage.bitermotors_runtime_visuals = storage.bitermotors_runtime_visuals or {}
  return storage.bitermotors_runtime_visuals
end

local function destroy_bitermotors_runtime_visual(unit_number)
  if not unit_number then return end
  local visuals = bitermotors_runtime_visuals()
...
local function attach_bitermotors_runtime_visual(entity)
  local config = entity and entity.valid and BITERMOTORS_RUNTIME_VISUAL_CONFIGS[entity.name]
  ...
  bitermotors_runtime_visuals()[entity.unit_number] = {
    object = object,
    entity = entity,
...
local function set_bitermotors_runtime_visual_enabled(entity, enabled)
  if not entity or not entity.valid or not entity.unit_number then return end
  local entry = bitermotors_runtime_visuals()[entity.unit_number]
  if not entry then
    attach_bitermotors_runtime_visual(entity)
    entry = bitermotors_runtime_visuals()[entity.unit_number]
  end
  if entry then entry.enabled = enabled == true end
end

local function rebuild_bitermotors_runtime_visuals()
  for unit_number in pairs(bitermotors_runtime_visuals()) do
    destroy_bitermotors_runtime_visual(unit_number)
  end
  local names = {}
  for name in pairs(BITERMOTORS_RUNTIME_VISUAL_CONFIGS) do names[#names + 1] = name end
...
local function update_bitermotors_runtime_visuals()
  local frame_index = math.floor(game.tick / 15) % 8 + 1
  for unit_number, entry in pairs(bitermotors_runtime_visuals()) do
    if not entry.entity or not entry.entity.valid or not entry.object or not entry.object.valid then
      ...
      if entry.status then
        local market_state = storage.bitermotors_sales_office_market_states
          and storage.bitermotors_sales_office_market_states[unit_number]
...
function charger_stall_visuals()
  storage.bitermotors_charger_stall_visuals = storage.bitermotors_charger_stall_visuals or {}
  return storage.bitermotors_charger_stall_visuals
end
...
function ensure_charger_stall_visuals(station)
  ...
  for stall_index = 1, config.stalls do
    ...
    entry.objects[stall_index] = rendering.draw_sprite{
      sprite = "bitermotors-charger-stall-idle-frame-1",
    ...
    if station.valid and station.unit_number then
      ...
      for stall_index, object in ipairs(entry and entry.objects or {}) do
        ...
        if object and object.valid then
          ...
          object.sprite = "bitermotors-charger-stall-" .. state .. "-frame-" .. staggered_frame
        end
      end
    end
  end
...
function sales_office_showroom_renderings()
  storage.bitermotors_sales_office_showroom_renderings =
    storage.bitermotors_sales_office_showroom_renderings or {}
  return storage.bitermotors_sales_office_showroom_renderings
end
...
local function find_stations(surface, force)
  return registered_bitermotors_entities("stations", force, surface)
end
...
function production_history_by_force()
  storage.bitermotors_production_history_by_force =
    storage.bitermotors_production_history_by_force or {}
...
function agi_training_unlocks()
  storage.bitermotors_agi_training_unlocks = storage.bitermotors_agi_training_unlocks or {}
  return storage.bitermotors_agi_training_unlocks
end

function bitermotors_compute_machines()
  storage.bitermotors_compute_machines = storage.bitermotors_compute_machines or {}
  return storage.bitermotors_compute_machines
end

function bitermotors_compute_power_failures()
  storage.bitermotors_compute_power_failures = storage.bitermotors_compute_power_failures or {}
  return storage.bitermotors_compute_power_failures
end

function bitermotors_compute_queue()
  storage.bitermotors_compute_queue = PowerQueue.ensure(storage.bitermotors_compute_queue)
...
function orbital_radiator_panels()
  ...
  return storage.bitermotors_orbital_radiator_panels
end

function mark_orbital_cooling_dirty()
  storage.bitermotors_orbital_cooling_dirty = true
end

function track_orbital_radiator_panel(entity)
  if entity and entity.valid and entity.name == ORBITAL_RADIATOR_PANEL_NAME
    and entity.unit_number then
    orbital_radiator_panels()[entity.unit_number] = entity
    mark_orbital_cooling_dirty()
  end
end

function refresh_orbital_cooling_assignments()
  local radiator_counts = {}
  for unit_number, radiator in pairs(orbital_radiator_panels()) do
    if not radiator or not radiator.valid then
      orbital_radiator_panels()[unit_number] = nil
    else
      local key = tostring(radiator.surface.index) .. ":" .. tostring(radiator.force.index)
      radiator_counts[key] = (radiator_counts[key] or 0) + 1
    end
  end

  local cores_by_group = {}
  for _, entity in pairs(bitermotors_compute_machines()) do
    if entity and entity.valid and entity.name == ORBITAL_DATACENTER_CORE_NAME then
      local key = tostring(entity.surface.index) .. ":" .. tostring(entity.force.index)
      cores_by_group[key] = cores_by_group[key] or {}
      cores_by_group[key][#cores_by_group[key] + 1] = entity.unit_number
    end
  end

  local assignments = {}
  for key, unit_numbers in pairs(cores_by_group) do
    table.sort(unit_numbers)
    local cooled_count = math.floor((radiator_counts[key] or 0) / ORBITAL_RADIATORS_PER_CORE)
    for index, unit_number in ipairs(unit_numbers) do
      assignments[unit_number] = index <= cooled_count
    end
  end
  storage.bitermotors_orbital_cooling_assignments = assignments
  storage.bitermotors_orbital_cooling_dirty = false
  return assignments
end

function orbital_core_has_cooling(entity)
  if not entity or not entity.valid or entity.name ~= ORBITAL_DATACENTER_CORE_NAME then
    return true
  end
  if storage.bitermotors_orbital_cooling_dirty
    or not storage.bitermotors_orbital_cooling_assignments then
    refresh_orbital_cooling_assignments()
  end
  return storage.bitermotors_orbital_cooling_assignments[entity.unit_number] == true
end

function orbital_cooling_status(force)
  if storage.bitermotors_orbital_cooling_dirty
    or not storage.bitermotors_orbital_cooling_assignments then
    refresh_orbital_cooling_assignments()
  end
  local cores = 0
  local cooled_cores = 0
  for unit_number, entity in pairs(bitermotors_compute_machines()) do
    if entity and entity.valid and entity.name == ORBITAL_DATACENTER_CORE_NAME
      and (not force or entity.force == force) then
      cores = cores + 1
      if storage.bitermotors_orbital_cooling_assignments[unit_number] then
        cooled_cores = cooled_cores + 1
      end
    end
  end
  local radiators = 0
  for _, entity in pairs(orbital_radiator_panels()) do
  ...
  return {
    cores = cores,
    cooled_cores = cooled_cores,
    radiators = radiators,
    required_radiators = cores * ORBITAL_RADIATORS_PER_CORE
  }
end
...
function track_bitermotors_compute_machine(entity)
  if entity and entity.valid and entity.unit_number and BITERMOTORS_COMPUTE_RECIPES[entity.name] then
    ...
    if entity.name == ORBITAL_DATACENTER_CORE_NAME then
      mark_orbital_cooling_dirty()
    end
  end
end
...
function rebuild_bitermotors_compute_machines()
  ...
  for _, surface in pairs(game.surfaces) do
    for _, entity in pairs(surface.find_entities_filtered{name = names}) do
      track_bitermotors_compute_machine(entity)
    end
    for _, entity in pairs(surface.find_entities_filtered{name = ORBITAL_RADIATOR_PANEL_NAME}) do
...
function reset_underpowered_compute_progress()
  ...
  while processed < 32 and #queue.units > 0 do
    ...
    if not entity or not entity.valid then
      ...
      local failure = bitermotors_compute_power_failures()[unit_number]
      if failure then
      ...
      local required_recipe = BITERMOTORS_COMPUTE_RECIPES[entity.name]
...
function ai_efficiency_progress()
  storage.bitermotors_ai_efficiency_progress = storage.bitermotors_ai_efficiency_progress or {}
  return storage.bitermotors_ai_efficiency_progress
end
...
function bitermotors_accelerated_start_enabled()
  local setting = settings.startup["bitermotors-accelerated-start"]
  return setting and setting.value == true
end

function bitermotors_copy_table(source)
  local copy = {}
...
function configure_bitermotors_new_game()
  if not bitermotors_accelerated_start_enabled() then
    return
  end
  local force = game.forces.player
  for _, technology_name in pairs(BITERMOTORS_START_TECHNOLOGIES) do
  ...
  if remote.interfaces.freeplay then
    remote.call("freeplay", "set_ship_items", bitermotors_copy_table(BITERMOTORS_START_SHIP_ITEMS))
    remote.call("freeplay", "set_debris_items", bitermotors_copy_table(BITERMOTORS_START_DEBRIS_ITEMS))
...
function grant_bitermotors_energy_jumpstart(player)
  if not bitermotors_accelerated_start_enabled() or not player or not player.valid then
    return nil
  end
  storage.bitermotors_energy_jumpstart_forces = storage.bitermotors_energy_jumpstart_forces or {}
  if storage.bitermotors_energy_jumpstart_forces[player.force.name] then
    return nil
  end
  ...
  for item_name, count in pairs(BITERMOTORS_ENERGY_JUMPSTART_ITEMS) do
    local quality = BITERMOTORS_ENERGY_JUMPSTART_NORMAL_QUALITY_ITEMS[item_name]
      and "normal"
      or BITERMOTORS_ENERGY_JUMPSTART_QUALITY
    inventory.insert{name = item_name, count = count, quality = quality}
  end
  pcall(function() chest.backer_name = "Captain's Chest" end)
  storage.bitermotors_energy_jumpstart_forces[player.force.name] = true
  return chest
end
...
function seed_crash_site_salvage(player)
  if not bitermotors_accelerated_start_enabled() or not player or not player.valid then return end
...
function electric_vehicle_registry()
  storage.bitermotors_electric_vehicles = storage.bitermotors_electric_vehicles or {}
  return storage.bitermotors_electric_vehicles
end
...
function rebuild_electric_vehicles()
  storage.bitermotors_electric_vehicles = {}
  for _, surface in pairs(game.surfaces) do
...
local function customer_markers()
  storage.bitermotors_customer_markers = storage.bitermotors_customer_markers or {}
  return storage.bitermotors_customer_markers
end

function customer_unit_registry()
  storage.bitermotors_customer_units = storage.bitermotors_customer_units or {}
  return storage.bitermotors_customer_units
end

function customer_settlement_populations()
  storage.bitermotors_customer_settlement_populations = storage.bitermotors_customer_settlement_populations or {}
  return storage.bitermotors_customer_settlement_populations
end

function customer_visible_count()
  if storage.bitermotors_customer_visible_count == nil then
    local count = 0
    for _, entity in pairs(customer_unit_registry()) do
      if entity and entity.valid then count = count + 1 end
    end
    storage.bitermotors_customer_visible_count = count
  end
  return storage.bitermotors_customer_visible_count
end

function customer_population_members()
  storage.bitermotors_customer_population_members = storage.bitermotors_customer_population_members or {}
  return storage.bitermotors_customer_population_members
end
...
function customer_home_settlements()
  storage.bitermotors_customer_home_settlements = storage.bitermotors_customer_home_settlements or {}
  return storage.bitermotors_customer_home_settlements
end
...
function customer_charging_commutes()
  storage.bitermotors_customer_charging_commutes = storage.bitermotors_customer_charging_commutes or {}
  return storage.bitermotors_customer_charging_commutes
end

function bitermotors_entity_registries()
...
function cybertrain_runtime()
  storage.bitermotors_cybertrain_runtime = storage.bitermotors_cybertrain_runtime or {
    semis = {}, cybertrain_order = {}, cybertrain_cursor = 1,
    stops = {}, stop_order = {}, stop_cursor = 1,
    batteries = {}, stop_power = {}, stop_ticks = {}
  }
  return storage.bitermotors_cybertrain_runtime
end

function track_cybertrain_runtime(entity, factory_new)
  if not entity or not entity.valid or not entity.unit_number then return end
  local runtime = cybertrain_runtime()
  if entity.name == CYBERTRAIN_NAME then
    if not runtime.semis[entity.unit_number] then
      runtime.semis[entity.unit_number] = entity
      runtime.cybertrain_order[#runtime.cybertrain_order + 1] = entity.unit_number
    end
    local battery = runtime.batteries[entity.unit_number]
    if not battery then
      runtime.batteries[entity.unit_number] = {
        energy = factory_new == false and 0 or CYBERTRAIN_BATTERY_CAPACITY,
  ...
  elseif entity.name == CYBERTRAIN_CHARGING_STOP_NAME and not runtime.stops[entity.unit_number] then
    runtime.stops[entity.unit_number] = entity
    runtime.stop_order[#runtime.stop_order + 1] = entity.unit_number
  end
end

function clear_cybertrain_drive_permission(entity)
...
function set_cybertrain_drive_permission(entity, enabled)
  if not entity or not entity.valid or not entity.burner then return end
  if not enabled then
    clear_cybertrain_drive_permission(entity)
    return
  end
  if not entity.burner.currently_burning
    and entity.burner.inventory.get_item_count(CYBERTRAIN_FUEL_NAME) == 0 then
    entity.burner.inventory.insert{name = CYBERTRAIN_FUEL_NAME, count = 1}
  end
end

function cybertrains_in_train(train)
  local result = {}
  ...
  for _, group in pairs(train.locomotives or {}) do
    for _, locomotive in pairs(group) do
      if locomotive.valid and locomotive.name == CYBERTRAIN_NAME then
...
function process_cybertrain(entity, battery)
  ...
  if battery.energy <= CYBERTRAIN_RESERVE_THRESHOLD and speed > CYBERTRAIN_RESERVE_SPEED then
    local train = entity.train
    if train and train.valid then
      train.speed = train.speed < 0 and -CYBERTRAIN_RESERVE_SPEED or CYBERTRAIN_RESERVE_SPEED
    end
    speed = CYBERTRAIN_RESERVE_SPEED
    battery.last_speed = math.min(battery.last_speed or speed, CYBERTRAIN_RESERVE_SPEED)
  end
  local train = entity.train
  local semis = cybertrains_in_train(train)
  local mass_share = train and train.valid and train.weight / math.max(1, #semis) or entity.prototype.weight
  local previous_kinetic = mass_share * (battery.last_speed or speed) ^ 2 * CYBERTRAIN_KINETIC_ENERGY_SCALE
  local current_kinetic = mass_share * speed ^ 2 * CYBERTRAIN_KINETIC_ENERGY_SCALE
  ...
  elseif kinetic_delta < 0 then
    local recovered = math.min(-kinetic_delta * CYBERTRAIN_REGEN_EFFICIENCY, CYBERTRAIN_BATTERY_CAPACITY - battery.energy)
  ...
  battery.energy = math.max(0, math.min(CYBERTRAIN_BATTERY_CAPACITY, battery.energy))
  battery.reserve_mode = battery.energy <= CYBERTRAIN_RESERVE_THRESHOLD
  if battery.reserve_mode and speed > CYBERTRAIN_RESERVE_SPEED then
    local train = entity.train
    if train and train.valid then
      train.speed = train.speed < 0 and -CYBERTRAIN_RESERVE_SPEED or CYBERTRAIN_RESERVE_SPEED
    end
  ...
  set_cybertrain_drive_permission(entity, true)
end

function ensure_cybertrain_charging_power(stop)
  local runtime = cybertrain_runtime()
  ...
  for _, candidate in pairs(stop.surface.find_entities_filtered{
    name = CYBERTRAIN_CHARGING_POWER_NAME, position = stop.position, radius = 0.25, force = stop.force
  }) do
  ...
  if not power then
    power = stop.surface.create_entity{
      name = CYBERTRAIN_CHARGING_POWER_NAME,
...
function process_cybertrain_charging_stop(stop)
  local runtime = cybertrain_runtime()
  local power = ensure_cybertrain_charging_power(stop)
  ...
  for _, semi in pairs(semis) do
    ...
    if battery and battery.energy < CYBERTRAIN_BATTERY_CAPACITY then charging[#charging + 1] = battery end
  end
  ...
  power.power_usage = CYBERTRAIN_CHARGING_POWER
  ...
  local available = CYBERTRAIN_CHARGING_POWER * tick_delta / 60 * power_factor
  for _, battery in pairs(charging) do
    local delivered = math.min(available / #charging, CYBERTRAIN_BATTERY_CAPACITY - battery.energy)
    battery.energy = battery.energy + delivered
  end
end

function process_bounded_cybertrain_queue(order, members, cursor_name, callback)
  local runtime = cybertrain_runtime()
  local attempts = 0
  local budget = math.min(#order, CYBERTRAIN_PROCESS_BUDGET)
...
function process_cybertrain_runtime()
  local runtime = cybertrain_runtime()
  process_bounded_cybertrain_queue(runtime.cybertrain_order, runtime.semis, "cybertrain_cursor", function(entity)
    local battery = runtime.batteries[entity.unit_number]
    if battery then process_cybertrain(entity, battery) end
  end)
  process_bounded_cybertrain_queue(runtime.stop_order, runtime.stops, "stop_cursor", process_cybertrain_charging_stop)
end

function rebuild_cybertrain_runtime()
  local old = storage.bitermotors_cybertrain_runtime
  storage.bitermotors_cybertrain_runtime = {
    semis = {}, cybertrain_order = {}, cybertrain_cursor = 1,
    stops = {}, stop_order = {}, stop_cursor = 1,
    batteries = old and old.batteries or {}, stop_power = {}, stop_ticks = {}
  }
  for _, surface in pairs(game.surfaces) do
    for _, entity in pairs(surface.find_entities_filtered{name = {CYBERTRAIN_NAME, CYBERTRAIN_CHARGING_STOP_NAME}}) do
      track_cybertrain_runtime(entity, true)
    end
  end
end

function rebuild_bitermotors_entity_registries()
  ...
  rebuild_bitermotors_registry_reconciliation_chunks()
end

function rebuild_bitermotors_registry_reconciliation_chunks()
  local state = {
    version = BITERMOTORS_REGISTRY_RECONCILIATION_VERSION,
    by_surface = {},
    surface_order = {},
    surface_cursor = 1
  }
  ...
  storage.bitermotors_registry_reconciliation = state
  return state
end

function bitermotors_registry_reconciliation_state()
  local state = storage.bitermotors_registry_reconciliation
  if not state or state.version ~= BITERMOTORS_REGISTRY_RECONCILIATION_VERSION then
    state = rebuild_bitermotors_registry_reconciliation_chunks()
  end
  return state
end

function register_bitermotors_reconciliation_chunk(surface, position)
  if not surface or not surface.valid or not position then return end
  local state = bitermotors_registry_reconciliation_state()
...
function next_bitermotors_reconciliation_chunk(state)
...
function reconcile_bitermotors_entity_registry_step()
  local reconciliation = bitermotors_registry_reconciliation_state()
  local performance_state = PerformanceState.ensure(storage)
  local discovered = 0
  local scanned = 0
  for _ = 1, BITERMOTORS_REGISTRY_RECONCILIATION_CHUNKS_PER_STEP do
    local surface, chunk = next_bitermotors_reconciliation_chunk(reconciliation)
    ...
    for _, entity in pairs(surface.find_entities_filtered{
      name = BITERMOTORS_REGISTRY_ENTITY_NAMES,
      area = area
    }) do
      ...
      if unit_number and not tracked then
        track_bitermotors_entity(entity)
        mark_bitermotors_market_dirty(entity.force, "registry-reconciled")
  ...
  storage.bitermotors_perf_counters.registry_reconciliation_steps =
    (storage.bitermotors_perf_counters.registry_reconciliation_steps or 0) + 1
  storage.bitermotors_perf_counters.registry_reconciliation_chunks =
    (storage.bitermotors_perf_counters.registry_reconciliation_chunks or 0) + scanned
  storage.bitermotors_perf_counters.registry_reconciliation_discoveries =
    (storage.bitermotors_perf_counters.registry_reconciliation_discoveries or 0) + discovered
end

function bitermotors_market_cache()
  return PerformanceState.ensure(storage).market_cache
end

function bitermotors_market_generation()
...
function customer_settlement_market_forces()
  storage.bitermotors_customer_settlement_market_forces = storage.bitermotors_customer_settlement_market_forces or {}
  return storage.bitermotors_customer_settlement_market_forces
end
...
function customer_commute_timing_wheel()
  storage.bitermotors_customer_commute_timing_wheel = TimingWheel.ensure(
    storage.bitermotors_customer_commute_timing_wheel,
    3600
  )
  return storage.bitermotors_customer_commute_timing_wheel
end

function customer_road_rage_states()
  storage.bitermotors_customer_road_rage_states = storage.bitermotors_customer_road_rage_states or {}
  return storage.bitermotors_customer_road_rage_states
end

function customer_road_rage_timing_wheel()
  storage.bitermotors_customer_road_rage_timing_wheel = TimingWheel.ensure(
    storage.bitermotors_customer_road_rage_timing_wheel,
    600
  )
  return storage.bitermotors_customer_road_rage_timing_wheel
end

function customer_active_commutes()
  storage.bitermotors_customer_active_commutes = storage.bitermotors_customer_active_commutes or {}
  return storage.bitermotors_customer_active_commutes
end
...
function rebuild_customer_commute_queue()
  storage.bitermotors_customer_commute_timing_wheel = TimingWheel.ensure(nil, 3600)
  storage.bitermotors_customer_active_commutes = {}
  for unit_number, ownership in pairs(customer_vehicle_owners()) do
...
function customer_commute_totals()
  storage.bitermotors_customer_commute_totals = storage.bitermotors_customer_commute_totals or {}
  return storage.bitermotors_customer_commute_totals
end

function office_buyer_reservations()
  storage.bitermotors_office_buyer_reservations = storage.bitermotors_office_buyer_reservations or {}
...
function sales_office_market_states()
  storage.bitermotors_sales_office_market_states = storage.bitermotors_sales_office_market_states or {}
  return storage.bitermotors_sales_office_market_states
end

function sales_office_market_alert_states()
  storage.bitermotors_sales_office_market_alert_states =
    storage.bitermotors_sales_office_market_alert_states or {}
  return storage.bitermotors_sales_office_market_alert_states
end

function buyer_reserved_by_unit()
  storage.bitermotors_buyer_reserved_by_unit = storage.bitermotors_buyer_reserved_by_unit or {}
  return storage.bitermotors_buyer_reserved_by_unit
end

function station_power_service()
  storage.bitermotors_station_power_service = storage.bitermotors_station_power_service or {}
  return storage.bitermotors_station_power_service
end

local function sales_office_coverage_enabled()
  storage.bitermotors_sales_office_coverage_enabled = storage.bitermotors_sales_office_coverage_enabled or {}
  return storage.bitermotors_sales_office_coverage_enabled
end

local function sales_office_coverage_renderings()
  storage.bitermotors_sales_office_coverage_renderings = storage.bitermotors_sales_office_coverage_renderings or {}
  return storage.bitermotors_sales_office_coverage_renderings
end
...
local function refresh_all_sales_office_coverage()
  ...
  storage.bitermotors_sales_office_coverage_dirty = false
end
...
local function nearby_real_power_pole(station)
  storage.bitermotors_station_power_pole_cache = storage.bitermotors_station_power_pole_cache or {}
...
local function remove_station_grid_connection(station)
  ...
  local connections = storage.bitermotors_station_grid_connections or {}
...
function apply_bitermotors_enemy_pressure_settings()
  ...
  map_settings.pollution.enemy_attack_pollution_consumption_modifier =
    BITERMOTORS_ENEMY_ATTACK_POLLUTION_COST
  map_settings.unit_group.max_gathering_unit_groups =
    BITERMOTORS_MAX_GATHERING_ATTACK_GROUPS
  map_settings.unit_group.max_unit_group_size = BITERMOTORS_MAX_ATTACK_GROUP_SIZE
  map_settings.enemy_expansion.min_expansion_cooldown =
    BITERMOTORS_MIN_EXPANSION_COOLDOWN_TICKS
  map_settings.enemy_expansion.max_expansion_cooldown =
    BITERMOTORS_MAX_EXPANSION_COOLDOWN_TICKS
  map_settings.enemy_evolution.pollution_factor =
    BITERMOTORS_POLLUTION_EVOLUTION_FACTOR
  storage.bitermotors_enemy_pressure_version = BITERMOTORS_ENEMY_PRESSURE_VERSION
  return true
end

function relieve_bitermotors_enemy_pressure(max_evolution)
...
function watch_customer_unit_destruction(entity)
  if not entity or not entity.valid or not entity.unit_number then return end
  storage.bitermotors_customer_destruction_watches =
    storage.bitermotors_customer_destruction_watches or {}
  if storage.bitermotors_customer_destruction_watches[entity.unit_number] then return end
  storage.bitermotors_customer_destruction_watches[entity.unit_number] =
...
function register_customer_unit(entity, settlement, market_force)
  ...
  mark_bitermotors_market_dirty(market_force, "customer-registered")
  return true
end
...
function unregister_customer_unit_number(unit_number)
  ...
  if storage.bitermotors_customer_destruction_watches then
    storage.bitermotors_customer_destruction_watches[unit_number] = nil
  end
  ...
  if market_force then mark_bitermotors_market_dirty(market_force, "customer-removed") end
  return ownership
end

function rebuild_customer_settlement_population_cache()
  local previous = storage.bitermotors_customer_settlement_populations or {}
  storage.bitermotors_customer_settlement_populations = {}
  storage.bitermotors_customer_population_members = {}
...
function reconcile_customer_lifecycle_state()
  if storage.bitermotors_customer_lifecycle_version == CUSTOMER_LIFECYCLE_VERSION then
  ...
  storage.bitermotors_perf_counters = storage.bitermotors_perf_counters or {}
  storage.bitermotors_perf_counters.customer_lifecycle_repairs =
    (storage.bitermotors_perf_counters.customer_lifecycle_repairs or 0) + 1
  storage.bitermotors_perf_counters.stale_customer_units_removed =
    (storage.bitermotors_perf_counters.stale_customer_units_removed or 0) + removed
...
function ev_sales_gate_announcements()
  storage.bitermotors_ev_sales_gate_announcements = storage.bitermotors_ev_sales_gate_announcements or {}
  return storage.bitermotors_ev_sales_gate_announcements
end
...
function advanced_battery_chemistry_gate_announcements()
  storage.bitermotors_advanced_battery_chemistry_gate_announcements =
    storage.bitermotors_advanced_battery_chemistry_gate_announcements or {}
  return storage.bitermotors_advanced_battery_chemistry_gate_announcements
end
...
function foundry_power_gate_announcements()
  storage.bitermotors_foundry_power_gate_announcements =
    storage.bitermotors_foundry_power_gate_announcements or {}
  return storage.bitermotors_foundry_power_gate_announcements
end
...
function customer_settlement_moods(force)
  storage.bitermotors_customer_settlement_moods = storage.bitermotors_customer_settlement_moods or {}
  storage.bitermotors_customer_settlement_moods[force.index] = storage.bitermotors_customer_settlement_moods[force.index] or {}
  return storage.bitermotors_customer_settlement_moods[force.index]
end

function customer_mood_random()
  local generator = storage.bitermotors_customer_mood_random
  if not generator or not generator.valid then
    generator = game.create_random_generator()
    storage.bitermotors_customer_mood_random = generator
  end
  return generator
end
...
customer_service_for_force = function(force, advance_mood)
  ...
  if cached and game.tick - cached.tick < CUSTOMER_MARKET_CACHE_TICKS
    ...
    storage.bitermotors_perf_counters = storage.bitermotors_perf_counters or {}
    storage.bitermotors_perf_counters.invalid_market_snapshot_rebuilds =
      (storage.bitermotors_perf_counters.invalid_market_snapshot_rebuilds or 0) + 1
    mark_bitermotors_market_dirty(force, "invalid-market-snapshot")
    generation = bitermotors_market_generation()[force.index] or 0
  end
  local service = {
    assignments = {},
  ...
  storage.bitermotors_perf_counters = storage.bitermotors_perf_counters or {}
  storage.bitermotors_perf_counters.market_snapshot_builds =
    (storage.bitermotors_perf_counters.market_snapshot_builds or 0) + 1
  bitermotors_market_cache()[force.index] = {
    tick = game.tick,
...
function charge_station_vehicles(station)
  local by_force = storage.bitermotors_station_vehicle_assignments or {}
  ...
  for index = 1, math.min(vehicle_stalls, #assignment.vehicles) do
    ...
    if charge_vehicle(vehicle, joules) > 0 then
      ...
      if vehicle and vehicle.valid and vehicle.unit_number then
        storage.bitermotors_vehicle_charge_activity = storage.bitermotors_vehicle_charge_activity or {}
        storage.bitermotors_vehicle_charge_activity[vehicle.unit_number] = game.tick
      end
    end
  end
  return charged
...
function ev_driver_overlay_states()
  storage.bitermotors_ev_driver_overlay_states = storage.bitermotors_ev_driver_overlay_states or {}
  return storage.bitermotors_ev_driver_overlay_states
end

function ev_battery_popup_states()
  storage.bitermotors_ev_battery_popups = storage.bitermotors_ev_battery_popups or {}
  return storage.bitermotors_ev_battery_popups
end
...
function show_ev_battery_popup(player, vehicle)
  if not player or not player.valid or not vehicle or not vehicle.valid
    or (not is_electric_vehicle(vehicle) and vehicle.name ~= CYBERTRAIN_NAME) then return end
  destroy_ev_battery_popup(player.index)
  local energy
  local capacity
  if vehicle.name == CYBERTRAIN_NAME then
    local battery = cybertrain_runtime().batteries[vehicle.unit_number]
    energy = battery and battery.energy or 0
    capacity = CYBERTRAIN_BATTERY_CAPACITY
  else
...
function create_ev_driver_overlay(player, vehicle)
  ...
  local state = {
    ...
    market_generation = bitermotors_market_generation()[vehicle.force.index] or 0,
  ...
  state.charge_icon = rendering.draw_sprite{
    sprite = "item/bitermotors-electric-drive-charge",
    surface = vehicle.surface,
    target = vehicle,
    target_offset = {0, -2.1},
    x_scale = 0.55,
...
function refresh_ev_driver_overlays()
  local activity = storage.bitermotors_vehicle_charge_activity or {}
  local connected = {}
  for _, player in pairs(game.connected_players) do
    ...
    if not is_electric_vehicle(vehicle) or not vehicle.unit_number then
      ...
      local market_changed = state
        and state.market_generation ~= (bitermotors_market_generation()[vehicle.force.index] or 0)
...
function player_driven_bitermotors_ev_near(event, victim)
...
function trigger_customer_road_rage(event)
  ...
  local vehicle, target, player = player_driven_bitermotors_ev_near(event, victim)
  if not vehicle then return false end
  local megatruck = vehicle.name == "bitermotors-megatruck"
...
function replace_customer_prospect_entity(entity)
  ...
  if storage.bitermotors_customer_destruction_watches then
    storage.bitermotors_customer_destruction_watches[old_unit_number] = nil
  end
...
function queue_customer_vehicle_variant_migration()
  ...
  storage.bitermotors_customer_vehicle_variant_queue = queue
  return #queue
end

function enqueue_customer_variant_migration(unit_number)
  if not unit_number then return end
  storage.bitermotors_customer_vehicle_variant_queue = storage.bitermotors_customer_vehicle_variant_queue or {}
  local queue = storage.bitermotors_customer_vehicle_variant_queue
  queue[#queue + 1] = unit_number
end

function process_customer_vehicle_variant_migration(limit)
  local queue = storage.bitermotors_customer_vehicle_variant_queue or {}
  local migrated = 0
...
function charger_placement_overlay_states()
  storage.bitermotors_charger_placement_overlay_states = storage.bitermotors_charger_placement_overlay_states or {}
  return storage.bitermotors_charger_placement_overlay_states
end

function charger_hover_overlay_states()
  storage.bitermotors_charger_hover_overlay_states = storage.bitermotors_charger_hover_overlay_states or {
    by_player = {},
    by_station = {}
  }
  return storage.bitermotors_charger_hover_overlay_states
end
...
function reset_charger_hover_overlays()
  ...
  storage.bitermotors_charger_hover_overlay_states = {
    by_player = {},
    by_station = {}
  }
end
...
function set_charger_placement_overlay(player, enabled)
  ...
  if not ok then
    ...
    if not storage.bitermotors_charger_overlay_warnings[player.index] then
      storage.bitermotors_charger_overlay_warnings[player.index] = true
...
function customer_settlement_alert_states()
  storage.bitermotors_customer_settlement_alert_states = storage.bitermotors_customer_settlement_alert_states or {}
  return storage.bitermotors_customer_settlement_alert_states
end

function customer_settlement_map_tag_states()
  storage.bitermotors_customer_settlement_map_tag_states =
    storage.bitermotors_customer_settlement_map_tag_states or {}
...
function ensure_seed_customer(settlement, market_force)
  ...
  if customer and register_customer_unit(customer, settlement, market_force) then
    ...
    mark_bitermotors_market_dirty(market_force, "settlement-seed-customer")
    return customer
  end
...
local function customer_growth_states()
  storage.bitermotors_customer_growth_states = storage.bitermotors_customer_growth_states or {}
...
local function customer_growth_random()
  ...
  if not generator or not generator.valid then
    generator = game.create_random_generator()
    storage.bitermotors_customer_growth_random = generator
  end
  return generator
end
...
function waiting_market_buyers_at_station(station, service)
  ...
  if stale_assignment then
    mark_bitermotors_market_dirty(station.force, "invalid-assigned-settlement")
  end
  return count
end
...
function opened_bitermotors_entities()
  storage.bitermotors_opened_entities = storage.bitermotors_opened_entities or {}
  return storage.bitermotors_opened_entities
end
...
function add_bitermotors_metric_table(parent, rows)
...
function add_bitermotors_status_strip(parent, caption, color)
...
local function show_station_info_panel(player, station)
  ...
  titlebar.add{
    type = "sprite-button",
    name = "bitermotors_station_info_close",
  ...
  local state_color = grid_connected and (power_percent > 0 and BITERMOTORS_STATE_COLORS.good or BITERMOTORS_STATE_COLORS.bad)
  ...
  add_bitermotors_metric_table(panel, {
  ...
  if not grid_connected or power_percent == 0 then
    summary, summary_color = "Connect and power this charger.", BITERMOTORS_STATE_COLORS.bad
  elseif underserved_here > 0 then
    summary, summary_color = string.format("%d EVs need charging capacity.", underserved_here), BITERMOTORS_STATE_COLORS.bad
  elseif hostile_settlements > 0 and covered_settlements == 0 then
    summary, summary_color = "Add Sales Office coverage.", BITERMOTORS_STATE_COLORS.warning
  elseif active_stalls < config.stalls then
    summary, summary_color = string.format("%d stalls available.", config.stalls - active_stalls), BITERMOTORS_STATE_COLORS.good
  else
    summary, summary_color = "Charger operating at capacity.", BITERMOTORS_STATE_COLORS.good
  end
...
local function reservation_print_progress()
  storage.bitermotors_reservation_print_progress = storage.bitermotors_reservation_print_progress or {}
  return storage.bitermotors_reservation_print_progress
end

local function generate_station_reservations(force, service)
  if not researched(force, "bitermotors-ev-charging-network") and not first_prototype_sale_unlocked(force) then
    return
  end
...
local function victory_forces()
  storage.bitermotors_victory_forces = storage.bitermotors_victory_forces or {}
  return storage.bitermotors_victory_forces
end

local function grid_controllers()
  storage.bitermotors_grid_controllers = storage.bitermotors_grid_controllers or {}
  return storage.bitermotors_grid_controllers
end

local function first_prototype_sales()
  storage.bitermotors_first_prototype_sales = storage.bitermotors_first_prototype_sales or {}
  return storage.bitermotors_first_prototype_sales
end

local function sales_office_products_finished()
  storage.bitermotors_sales_office_products_finished = storage.bitermotors_sales_office_products_finished or {}
...
local function first_ev_production_line_hints()
  storage.bitermotors_first_ev_production_line_hints = storage.bitermotors_first_ev_production_line_hints or {}
  return storage.bitermotors_first_ev_production_line_hints
end

local function first_premium_ev_sales()
  storage.bitermotors_first_premium_ev_sales = storage.bitermotors_first_premium_ev_sales or {}
  return storage.bitermotors_first_premium_ev_sales
end

local function first_mass_market_ev_sales()
  storage.bitermotors_first_mass_market_ev_sales = storage.bitermotors_first_mass_market_ev_sales or {}
...
local function first_entity_placement_hints()
  storage.bitermotors_first_entity_placement_hints = storage.bitermotors_first_entity_placement_hints or {}
  return storage.bitermotors_first_entity_placement_hints
end
...
local function rebuild_grid_controllers()
  storage.bitermotors_grid_controllers = {}
  for _, surface in pairs(game.surfaces) do
...
local function is_bitermotors_name(name)
  return type(name) == "string" and string.sub(name, 1, 2) == "bitermotors-"
end

local function repair_researched_bitermotors_unlocks(force)
  local repaired = {}
  if not force or not force.valid then
    return repaired
  end
...
local function sync_force_unlocks(force)
  ...
  local v4_recipe = force.recipes and force.recipes["bitermotors-ev-charging-station-v4"]
  if v4_recipe then
    v4_recipe.enabled = researched(force, "bitermotors-autonomous-logistics")
  end
  if researched(force, "bitermotors-sales-office") then
...
function sync_grid_battery_sales_offices()
  ...
  local coverage_recipes = storage.bitermotors_sales_office_coverage_recipes
  local seen_offices = {}
  for _, office in pairs(registered_bitermotors_entities("sales_offices")) do
    if office.valid and office.unit_number then
...
function sales_office_buyer_status(office)
  ...
  if stale_snapshot then mark_bitermotors_market_dirty(office.force, "invalid-market-settlement") end
  local available = 0
  local customers = 0
  local owned = 0
  local capacity = 0
...
function update_sales_office_market_alerts()
  ...
  for _, player in pairs(game.connected_players) do
    ...
    local active = {}
    for _, office in pairs(registered_bitermotors_entities("sales_offices", player.force)) do
...
local function progress_snapshot(force)
  ...
  local first_sale_complete = first_ev_production_line_hints()[force.name] == true
    or researched(force, "bitermotors-premium-ev-program")
  local premium_sale_complete = first_premium_ev_sales()[force.name] == true
    or researched(force, "bitermotors-capital-scaling")
  local terrestrial_ai = ai_efficiency_track_status(force, "terrestrial")
  local orbital_ai = ai_efficiency_track_status(force, "orbital")
  local orbital_cooling = orbital_cooling_status(force)
  ...
  return {
    ...
    sales_office_researched = researched(force, "bitermotors-sales-office"),
    ev_production_researched = researched(force, "bitermotors-premium-ev-program"),
    ...
    charging_network_researched = researched(force, "bitermotors-ev-charging-network"),
    mass_market_researched = researched(force, "bitermotors-capital-scaling"),
    energy_products_researched = researched(force, "bitermotors-energy-products"),
    ...
    nickel_ore_mined = count_item_produced(force, "bitermotors-nickel-ore"),
    lithium_brine_pumped = count_fluid_produced(force, "bitermotors-lithium-brine"),
    acidic_tailings_produced = count_fluid_produced(force, "bitermotors-acidic-tailings"),
    nickel_sulfate_produced = count_item_produced(force, "bitermotors-nickel-sulfate"),
    lithium_carbonate_produced = count_item_produced(force, "bitermotors-lithium-carbonate"),
    high_nickel_cells_produced = count_item_produced(force, "bitermotors-high-nickel-cell"),
    high_energy_battery_packs_produced = count_item_produced(force, "bitermotors-high-energy-battery-pack"),
    lfp_cells_produced = count_item_produced(force, "bitermotors-lfp-cell"),
    lfp_battery_packs_produced = count_item_produced(force, "bitermotors-lfp-battery-pack"),
    ...
    roadsters_sold = sold["bitermotors-prototype-roadster"] or 0,
    premium_evs_sold = sold["bitermotors-premium-ev"] or 0,
    mass_market_evs_sold = sold["bitermotors-mass-market-ev"] or 0,
    megatrucks_sold = sold["bitermotors-megatruck"] or 0,
    ...
    grid_controllers = count_entities(force, "bitermotors-planetary-grid-controller"),
...
local function current_progress_objective(snapshot)
  ...
  elseif snapshot.cooled_orbital_datacenter_cores < snapshot.orbital_datacenter_cores then
    return "Orbital cooling", "Install eight Orbital Radiator Panels per Datacenter Core.", string.format(
      ...
      snapshot.orbital_radiator_panels,
...
function progress_objective_icon(stage)
  local icons = {
    ["Customer discovery"] = "item/bitermotors-sales-office",
    ["Prototype revenue"] = "item/bitermotors-prototype-roadster",
    ["Prototype market validation"] = "item/bitermotors-prototype-roadster",
    ["Battery breakthrough"] = "item/bitermotors-high-energy-battery-pack",
    ["Battery minerals"] = "item/bitermotors-nickel-ore",
    ["Battery refining"] = "item/bitermotors-nickel-sulfate",
    ...
    ["Industrial electrification"] = "item/bitermotors-high-density-solar-array",
    ...
    ["AGI infrastructure"] = "item/bitermotors-planetary-grid-controller",
    ["AGI scale"] = "item/bitermotors-ai-token",
    ["AGI training"] = "item/bitermotors-agi-training-dataset",
    ["AGI achieved"] = "item/bitermotors-agi-model"
  }
  return icons[stage] or "item/bitermotors-dollar"
end

function progress_health(snapshot)
  if snapshot.victory then
    return "Complete", "AGI achieved.", BITERMOTORS_STATE_COLORS.good
  elseif snapshot.sales_office_researched and snapshot.sales_offices == 0 then
    return "Blocked", "Build a Sales Office to open the customer market.", BITERMOTORS_STATE_COLORS.bad
  elseif snapshot.sales_offices > 0 and snapshot.customer_settlements == 0 then
    return "Blocked", "No customer settlements are covered.", BITERMOTORS_STATE_COLORS.bad
  elseif snapshot.customer_settlements > 0 and snapshot.powered_stations == 0 then
    return "Blocked", "Customer settlements need a powered charger.", BITERMOTORS_STATE_COLORS.bad
  ...
  elseif snapshot.orbital_compute_researched
    and snapshot.cooled_orbital_datacenter_cores < snapshot.orbital_datacenter_cores then
    return "Cooling alert", string.format(
      "%d orbital cores lack radiator capacity.",
      snapshot.orbital_datacenter_cores - snapshot.cooled_orbital_datacenter_cores
    ), BITERMOTORS_STATE_COLORS.bad
  elseif snapshot.powered_stations > 0 and snapshot.first_sale_complete then
    return "Operating", "Customer market and charging are online.", BITERMOTORS_STATE_COLORS.good
  elseif snapshot.powered_stations > 0 then
    return "Ready", "Charging is online; complete the first sale.", BITERMOTORS_STATE_COLORS.good
  end
  return "Next milestone", "Build the terrestrial business.", BITERMOTORS_STATE_COLORS.warning
end

function current_progress_measure(snapshot)
  if snapshot.victory then return "AGI training", 1, 1 end
  if snapshot.orbital_compute_researched
...
function add_progress_metrics(parent, rows)
  ...
  for _, row in ipairs(rows) do
    local status = row.color == BITERMOTORS_STATE_COLORS.bad and "Red: action is required."
      or row.color == BITERMOTORS_STATE_COLORS.warning and "Orange: prepare or keep progressing."
      or row.color == BITERMOTORS_STATE_COLORS.good and "Green: healthy or complete."
...
local function refresh_progress_panel(player)
  ...
  local signature = table.concat(signature_parts, "|")
  if storage.bitermotors_progress_panel_signatures[player.index] == signature then
    return
  end
  storage.bitermotors_progress_panel_signatures[player.index] = signature
  ...
  stage_label.style.font_color = BITERMOTORS_STATE_COLORS.warning
  ...
  if measure_name then
    ...
    progress_value.style.font_color = measure_current >= measure_target
      and BITERMOTORS_STATE_COLORS.good or BITERMOTORS_STATE_COLORS.warning
  ...
  if snapshot.industrial_supply_chain_researched or snapshot.big_mining_drill_researched
    ...
    industry_rows[#industry_rows + 1] = {
      ...
      color = snapshot.industrial_supply_chain_researched and BITERMOTORS_STATE_COLORS.good
  ...
  if snapshot.big_mining_drill_researched then
    industry_rows[#industry_rows + 1] = {
      ...
      color = snapshot.big_mining_drills > 0 and BITERMOTORS_STATE_COLORS.good or BITERMOTORS_STATE_COLORS.warning,
  ...
  if snapshot.foundry_researched then
    industry_rows[#industry_rows + 1] = {
      ...
      color = snapshot.foundries > 0 and BITERMOTORS_STATE_COLORS.good or BITERMOTORS_STATE_COLORS.warning,
  ...
  if snapshot.logistic_system_available or snapshot.logistic_system_researched then
    industry_rows[#industry_rows + 1] = {
      ...
      color = snapshot.logistic_system_researched and BITERMOTORS_STATE_COLORS.good
        or BITERMOTORS_STATE_COLORS.warning,
  ...
  if snapshot.charging_capacity > 0 or snapshot.requested_customer_stalls > 0 then
    ...
    local power_color = demand_kw <= 0 and BITERMOTORS_STATE_COLORS.neutral
      or (served_kw >= demand_kw * 0.95 and BITERMOTORS_STATE_COLORS.good
        or BITERMOTORS_STATE_COLORS.bad)
    ...
    grid_rows[#grid_rows + 1] = {
      sprite = "item/bitermotors-ev-charging-station", label = "Charging stalls",
      value = string.format(
        "%d / %d powered",
    ...
    grid_rows[#grid_rows + 1] = {
      sprite = "item/bitermotors-mass-market-ev", label = "Powered capacity",
      value = string.format(
        "%d EVs; %d spare",
  ...
  if snapshot.first_sale_complete then
    ...
    grid_rows[#grid_rows + 1] = {
      sprite = "item/bitermotors-premium-ev", label = "EV owners",
      ...
      color = snapshot.stranded_evs > 0 and BITERMOTORS_STATE_COLORS.bad or BITERMOTORS_STATE_COLORS.good,
    ...
    if step and step.available then
      grid_rows[#grid_rows + 1] = {
        sprite = "item/bitermotors-ev-charging-station", label = "Next charger activation",
        value = string.format(
          "%d EV sale%s: +%.0f kW",
        ...
        color = BITERMOTORS_STATE_COLORS.neutral,
    ...
    elseif step and step.needs_charger then
      grid_rows[#grid_rows + 1] = {
        ...
        color = snapshot.stranded_evs > 0 and BITERMOTORS_STATE_COLORS.bad
          or BITERMOTORS_STATE_COLORS.warning,
  ...
  if snapshot.sales_office_researched or snapshot.sales_offices > 0 then
    ...
    market_rows[#market_rows + 1] = {
      ...
      color = snapshot.customer_settlements > 0 and BITERMOTORS_STATE_COLORS.good or BITERMOTORS_STATE_COLORS.bad,
  ...
  if snapshot.charging_capacity > 0 or snapshot.powered_stations > 0 then
    market_rows[#market_rows + 1] = {
      ...
      color = snapshot.powered_stations == 0 and BITERMOTORS_STATE_COLORS.bad
        or (snapshot.active_stalls > 0 and BITERMOTORS_STATE_COLORS.good or BITERMOTORS_STATE_COLORS.warning),
    ...
    market_rows[#market_rows + 1] = {
      sprite = "item/bitermotors-ev-reservation", label = "EV Reservations",
  ...
  if snapshot.prototype_evs_produced > 0 or snapshot.first_sale_complete then
    market_rows[#market_rows + 1] = {
      sprite = "item/bitermotors-prototype-roadster", label = "Roadsters sold",
      value = string.format("%d / 50", snapshot.roadsters_sold),
      color = snapshot.roadsters_sold >= 50 and BITERMOTORS_STATE_COLORS.good or BITERMOTORS_STATE_COLORS.warning,
  ...
  if snapshot.ev_production_researched then
    market_rows[#market_rows + 1] = {
      sprite = "item/bitermotors-premium-ev", label = "Premium EVs",
  ...
  if snapshot.mass_market_researched then
    market_rows[#market_rows + 1] = {
      sprite = "item/bitermotors-mass-market-ev", label = "Mass-market EVs",
      value = string.format("%d / 2,000 sold", snapshot.mass_market_evs_sold),
      color = snapshot.mass_market_evs_sold >= 2000 and BITERMOTORS_STATE_COLORS.good or BITERMOTORS_STATE_COLORS.warning,
  ...
  if snapshot.first_sale_complete then
    market_rows[#market_rows + 1] = {
      sprite = "item/bitermotors-mass-market-ev", label = "Active customer EVs", value = tostring(snapshot.customer_ev_fleet),
    ...
    market_rows[#market_rows + 1] = {
      sprite = "item/bitermotors-ev-charging-station", label = "Charging service",
    ...
    if snapshot.angry_settlements > 0 then
      market_rows[#market_rows + 1] = {
        ...
        value = tostring(snapshot.angry_settlements), color = BITERMOTORS_STATE_COLORS.bad,
    ...
    if snapshot.customer_commutes_en_route > 0 or snapshot.customer_commutes_charging > 0
      ...
      market_rows[#market_rows + 1] = {
        sprite = "item/bitermotors-prototype-roadster", label = "Charging commutes",
  ...
  if snapshot.orbital_compute_researched then
    compute_rows[#compute_rows + 1] = {
      ...
      color = snapshot.orbital_datacenter_cores > 0
        and snapshot.cooled_orbital_datacenter_cores == snapshot.orbital_datacenter_cores
        and BITERMOTORS_STATE_COLORS.good or BITERMOTORS_STATE_COLORS.bad,
  ...
  for _, stage_info in ipairs(progress_stages(snapshot)) do
    journey_rows[#journey_rows + 1] = {
      ...
      color = stage_info.complete and BITERMOTORS_STATE_COLORS.good or BITERMOTORS_STATE_COLORS.warning,
...
local function open_progress_panel(player)
  ...
  if existing then
    existing.destroy()
    storage.bitermotors_progress_panel_signatures = storage.bitermotors_progress_panel_signatures or {}
    storage.bitermotors_progress_panel_signatures[player.index] = nil
    return
  end
  local panel = player.gui.screen.add{
    type = "frame",
  ...
  storage.bitermotors_progress_refresh_ticks = storage.bitermotors_progress_refresh_ticks or {}
  storage.bitermotors_progress_refresh_ticks[player.index] = game.tick
end
...
local function entity_status_text(entity)
  ...
  if entity.name == ORBITAL_DATACENTER_CORE_NAME and not orbital_core_has_cooling(entity) then
    return "Cooling offline"
  end
  local status = entity.status
...
function entity_status_presentation(entity)
  if entity.name == SALES_OFFICE_NAME and entity.disabled_by_script then
    ...
    if market.kind == "no-market" then
      return "No customer settlements", BITERMOTORS_STATE_COLORS.bad
    elseif market.kind == "saturated" then
      if market.surplus_office then
        return "Surplus Sales Office", BITERMOTORS_STATE_COLORS.warning
      end
      return "Waiting for market growth", BITERMOTORS_STATE_COLORS.neutral
    elseif market.kind == "service-blocked" then
      return "Prospects need charging", BITERMOTORS_STATE_COLORS.bad
    elseif market.kind == "committed" then
      if market.surplus_office then
        return "Surplus Sales Office", BITERMOTORS_STATE_COLORS.warning
      end
      return "Prospects reserved", BITERMOTORS_STATE_COLORS.neutral
    elseif market.kind == "unavailable" then
      return "Refreshing prospects", BITERMOTORS_STATE_COLORS.warning
  ...
  elseif status == defines.entity_status.no_power or status == defines.entity_status.low_power then
    return entity_status_text(entity), BITERMOTORS_STATE_COLORS.bad
  elseif status == defines.entity_status.no_ingredients
    or status == defines.entity_status.item_ingredient_shortage
    ...
    return entity_status_text(entity), BITERMOTORS_STATE_COLORS.bad
  elseif status == defines.entity_status.no_recipe then
    return entity_status_text(entity), BITERMOTORS_STATE_COLORS.warning
  end
  return entity_status_text(entity), BITERMOTORS_STATE_COLORS.neutral
end
...
local function show_manufacturer_info_panel(player, entity)
  ...
  if not is_bitermotors_manufacturer(entity) then
    return
  end
  ...
  local panel = player.gui.relative.add{
    ...
    direction = "vertical",
    anchor = bitermotors_relative_anchor(entity)
  }
  panel.style.width = 380
  ...
  if entity.name == SALES_OFFICE_NAME then
    ...
    if recipe and recipe.name == GRID_BATTERY_SALE_RECIPE then
      ...
      if trip and trip.phase == "to_office" then
        trip_text, trip_color = "Buyer travelling to office", BITERMOTORS_STATE_COLORS.warning
      elseif trip and trip.phase == "waiting_product" then
        trip_text, trip_color = "Buyer waiting in showroom", BITERMOTORS_STATE_COLORS.good
      end
      local next_wave = "-"
      ...
      add_bitermotors_metric_table(panel, {
        ...
        },
        {
          ...
          color = energy.adoption_percent >= 99.9 and BITERMOTORS_STATE_COLORS.good
            or BITERMOTORS_STATE_COLORS.warning
        },
        {
          sprite = "item/bitermotors-ev-reservation",
          label = "Next referral wave",
          value = next_wave,
          color = energy.next_referral_tick and BITERMOTORS_STATE_COLORS.warning
            or BITERMOTORS_STATE_COLORS.neutral
        }
      })
      local summary
      ...
      if energy.settlements == 0 then
        summary, summary_color = "No customer settlements within the 384-tile energy market.", BITERMOTORS_STATE_COLORS.bad
      elseif entity.status == defines.entity_status.no_power
        or entity.status == defines.entity_status.low_power then
        summary, summary_color = "Restore Sales Office power.", BITERMOTORS_STATE_COLORS.bad
      ...
      elseif energy.waiting == 0 then
        summary, summary_color = "Waiting for the next referral wave.", BITERMOTORS_STATE_COLORS.neutral
      else
        summary, summary_color = "Waiting for a physical energy buyer.", BITERMOTORS_STATE_COLORS.warning
      end
      add_bitermotors_status_strip(panel, summary, summary_color)
      return
    end
    ...
    local prospect_color = BITERMOTORS_STATE_COLORS.good
    if market_state.kind == "service-blocked" or market_state.kind == "no-market" then
      prospect_color = BITERMOTORS_STATE_COLORS.bad
    elseif market_state.kind == "unavailable"
      ...
      prospect_color = BITERMOTORS_STATE_COLORS.warning
    elseif market_state.kind == "saturated" or market_state.kind == "committed" then
      prospect_color = BITERMOTORS_STATE_COLORS.neutral
    end
    ...
    local metric_rows = {
      ...
      {
        sprite = "item/bitermotors-ev-charging-station",
        label = "Charging",
        value = string.format(
          "%d EVs; %d spare",
      ...
      {sprite = "item/bitermotors-ev-charging-station", label = "Underserved", value = tostring(buyer_status.underserved), color = buyer_status.underserved > 0 and BITERMOTORS_STATE_COLORS.bad or BITERMOTORS_STATE_COLORS.good},
      {sprite = "item/bitermotors-ev-reservation", label = "Reserved", value = sale and string.format("%d / %d", reserved, sale.vehicles) or "-"}
    }
    ...
    elseif next_step.needs_charger then
      metric_rows[#metric_rows + 1] = {
        ...
        color = buyer_status.underserved > 0 and BITERMOTORS_STATE_COLORS.bad
          or BITERMOTORS_STATE_COLORS.warning
      }
    end
    add_bitermotors_metric_table(panel, metric_rows)

    local summary
    local summary_color
    if not recipe then
      summary, summary_color = "Select a sales contract.", BITERMOTORS_STATE_COLORS.warning
    ...
    elseif entity.disabled_by_script then
      if market_state.kind == "saturated" then
        if market_state.surplus_office then
          ...
          summary_color = BITERMOTORS_STATE_COLORS.warning
        else
          summary = "Market saturated. Keep this office for future growth or expand sales elsewhere."
          summary_color = BITERMOTORS_STATE_COLORS.neutral
        end
      elseif market_state.kind == "service-blocked" then
        ...
        summary_color = BITERMOTORS_STATE_COLORS.bad
    ...
    elseif entity.status == defines.entity_status.working then
      if market_state.surplus_office and market_state.kind == "committed" then
        summary, summary_color = string.format(
          ...
          market_state.remaining
        ), BITERMOTORS_STATE_COLORS.warning
      elseif market_state.surplus_office and market_state.kind == "low" then
        ...
        summary, summary_color = "Sales active.", BITERMOTORS_STATE_COLORS.good
      end
    else
      summary, summary_color = "Waiting for product inputs.", BITERMOTORS_STATE_COLORS.neutral
    end
    add_bitermotors_status_strip(panel, summary, summary_color)
    return
  else
...
function refresh_bitermotors_infrastructure_change(entity)
  ...
  for _, player in pairs(entity.force.connected_players) do
    local opened = opened_bitermotors_entities()[player.index]
    if is_station(opened) then
      close_entity_info_panel(player)
      show_station_info_panel(player, opened)
    elseif is_bitermotors_manufacturer(opened) then
      close_station_info_panel(player)
...
script.on_init(function()
  ...
  rebuild_bitermotors_compute_machines()
  ...
  rebuild_bitermotors_runtime_visuals()
...
script.on_event(defines.events.on_lua_shortcut, function(event)
  ...
  if event.prototype_name == BITERMOTORS_PROGRESS_SHORTCUT then
...
function update_ev_reverse_warnings()
  storage.bitermotors_ev_reverse_warning_tick = storage.bitermotors_ev_reverse_warning_tick or {}
  for _, player in pairs(game.connected_players) do
    local vehicle = player.vehicle
    ...
    local last_tick = storage.bitermotors_ev_reverse_warning_tick[player.index] or -60
    if reversing and game.tick - last_tick >= 60 then
      vehicle.surface.play_sound({
        path = "bitermotors-ev-reverse-warning",
        position = vehicle.position
      })
      storage.bitermotors_ev_reverse_warning_tick[player.index] = game.tick
    elseif not reversing then
      storage.bitermotors_ev_reverse_warning_tick[player.index] = nil
    end
  end
end
...
script.on_event(defines.events.on_chunk_generated, function(event)
  register_bitermotors_reconciliation_chunk(event.surface, event.position)
end)
...
script.on_event(defines.events.on_player_removed, function(event)
  ...
  storage.bitermotors_progress_panel_signatures = storage.bitermotors_progress_panel_signatures or {}
  storage.bitermotors_progress_panel_signatures[event.player_index] = nil
  storage.bitermotors_progress_refresh_ticks = storage.bitermotors_progress_refresh_ticks or {}
  storage.bitermotors_progress_refresh_ticks[event.player_index] = nil
end)
...
script.on_event(defines.events.on_research_finished, function(event)
  ...
  if research and research.name == "bitermotors-premium-ev-program" then
    announce_ev_production_line_researched(research.force)
  elseif research and research.name == "bitermotors-ev-charging-network" then
    announce_ev_charging_network_researched(research.force)
  elseif research and research.name == "bitermotors-capital-scaling" then
...
script.on_event(defines.events.on_gui_opened, function(event)
  ...
  if player and player.valid then
    ...
    if is_station(entity) then
      player.opened = nil
      opened_bitermotors_entities()[player.index] = entity
      close_entity_info_panel(player)
      show_station_info_panel(player, entity)
    elseif is_bitermotors_manufacturer(entity) then
      opened_bitermotors_entities()[player.index] = entity
      close_station_info_panel(player)
...
script.on_nth_tick(6, process_cybertrain_runtime)

script.on_nth_tick(30, function()
  reconcile_bitermotors_entity_registry_step()
  update_bitermotors_runtime_visuals()
  update_charger_stall_visuals()
  update_sales_office_showrooms()
  refresh_ev_driver_overlays()
  ...
  for _, player in pairs(game.connected_players) do
    ...
    if player.gui.screen[PROGRESS_PANEL_NAME]
      and UiRefresh.should_refresh_progress(
        storage.bitermotors_progress_refresh_ticks[player.index],
        game.tick
      ) then
      refresh_progress_panel(player)
      storage.bitermotors_progress_refresh_ticks[player.index] = game.tick
...
script.on_nth_tick(600, function()
  if storage.bitermotors_enemy_pressure_version ~= BITERMOTORS_ENEMY_PRESSURE_VERSION then
    apply_bitermotors_enemy_pressure_settings()
  end
...
remote.add_interface("bitermotors", {
  ...
  cybertrain_status = function(force_name)
    local force = game.forces[force_name or "player"]
    if not force then return nil end
    local runtime = cybertrain_runtime()
    local result = {}
    for unit_number, semi in pairs(runtime.semis) do
      if semi and semi.valid and semi.force == force then
        ...
        result[#result + 1] = {
          ...
          battery_percent = math.floor((battery.energy or 0) * 1000 / CYBERTRAIN_BATTERY_CAPACITY + 0.5) / 10,
          ...
          reserve_speed_limit = CYBERTRAIN_RESERVE_SPEED,
          speed = semi.speed
        }
      end
    end
  ...
  test_cybertrain_reserve = function(unit_number, energy, speed)
    if not script.active_mods["bitermotors_smoke"] then return false end
    local runtime = cybertrain_runtime()
    ...
    battery.energy = math.max(0, math.min(CYBERTRAIN_BATTERY_CAPACITY, energy or 0))
    ...
    process_cybertrain(semi, battery)
    return {
      reserve_mode = battery.reserve_mode == true,
      speed = math.abs(semi.speed or 0),
      speed_limit = CYBERTRAIN_RESERVE_SPEED,
      has_drive_permission = semi.burner and (
        semi.burner.currently_burning ~= nil
          or semi.burner.inventory.get_item_count(CYBERTRAIN_FUEL_NAME) > 0
      ) or false
    }
  end,
  ...
  performance_test_seed_owner = function(entity, spawner, force_name, vehicle_name, due_tick)
    if not script.active_mods["bitermotors_perf_benchmark"] then return false end
    local force = game.forces[force_name or "player"]
    ...
    add_customer_vehicle_ownership(entity.unit_number, {
      vehicle = vehicle_name or "bitermotors-mass-market-ev",
  ...
  customer_vehicle_ownership = function(force_name)
    ...
    return {
      ...
      last_assignment = storage.bitermotors_last_vehicle_sale_assignment
    }
  end,
  ...
  progression_integrity = function(force_name)
    ...
    local repaired = repair_researched_bitermotors_unlocks(force)
    sync_force_unlocks(force)
...
commands.add_command("bitermotors-status", "Open or report Biter Motors progression status.", function(command)
...
commands.add_command("bitermotors-note", "Record a timestamped Biter Motors playtest note.", function(command)
  local player = command.player_index and game.get_player(command.player_index)
  local text = command.parameter and string.gsub(command.parameter, "^%s*(.-)%s*$", "%1") or ""
  if not player or text == "" then
    if player then player.print("Usage: /bitermotors-note <observation>") end
    return
  end
  local snapshot = progress_snapshot(player.force)
  local stage = current_progress_objective(snapshot)
  helpers.write_file("bitermotors-playtest-notes.jsonl", helpers.table_to_json{
    tick = game.tick,
    player = player.name,
...
commands.add_command("bitermotors-coverage", "Report Biter Motors EV charging grid connections.", function(command)
  local player = command.player_index and game.get_player(command.player_index)
  local force = player and player.force or game.forces.player
