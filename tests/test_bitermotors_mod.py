...
class BiterMotorsModTest(unittest.TestCase):
    ...
    def test_bitermotors_charging_network_sales_loop(self):
        ...
        self.assertIn("local station_box = station.bounding_box", control)
        self.assertIn("station_box.right_bottom.x >= supply_left", control)
    ...
    def test_bitermotors_progress_interface_is_live_and_actionable(self):
        ...
        self.assertIn("orbital_complete = autonomy_complete and snapshot.planetary_grid_researched", journey)
        self.assertIn("complete = orbital_complete and snapshot.victory", journey)
        self.assertIn("journey = progress_stages(snapshot)", control)
    ...
    def test_bitermotors_control_generates_physical_reservations_at_chargers(self):
        ...
        self.assertIn("RESERVATION_BUFFER_LIMIT = 20", control)
        self.assertIn("RESERVATION_SAMPLES_PER_PRINT = 60", control)
        self.assertIn('require("runtime.reservation_planner")', control)
        self.assertIn("sales_office_reservation_capacity_per_minute", control)
        self.assertIn("reservation_print_plan", control)
        self.assertIn("reservation_plan.maximum_rate", control)
        self.assertIn("uncapped_reservations_per_minute", control)
        self.assertIn("RESERVATION_SAMPLES_PER_PRINT - 0.001", control)
        planner = (MOD / "runtime/reservation_planner.lua").read_text()
        self.assertIn("function ReservationPlanner.allocate", planner)
        self.assertIn("capped_rate = math.min(uncapped_rate", planner)
        self.assertIn("rates_by_station[station.key]", planner)
        validator = (ROOT / "scripts" / "validate-bitermotors-mod.sh").read_text()
        self.assertIn("reservation output exceeded active Sales Office throughput", validator)
        self.assertIn("bitermotors_reservation_print_progress", control)
        self.assertIn("station_reservation_rate_per_minute", control)
        self.assertIn('label = "Prospects"', control)
        self.assertIn("Reservation printing is shared across chargers and capped by active Sales Office throughput", control)
    ...
    def test_bitertaxi_depot_economy(self):
        ...
        depot_prototype = data[data.index("local bitertaxi_depot ="):data.index("local bitertaxi_depot_power =")]
        self.assertIn('table.deepcopy(data.raw.container["steel-chest"])', depot_prototype)
        self.assertNotIn('data.raw["logistic-container"]', depot_prototype)
        self.assertNotIn("logistic_mode", depot_prototype)
        ...
        self.assertIn('(entity.type == "container" or entity.type == "logistic-container")', control)
        ...
        self.assertIn("private fleet stacks", locale)
        validator = (ROOT / "scripts" / "validate-bitermotors-mod.sh").read_text()
        self.assertIn('data["container"]', validator)
        self.assertIn('data.get("logistic-container", {})', validator)
